...
| 옵션 | 기본값 | 설명 |
|---|---|---|
| `--feature-set` | `paper` | `paper`: 논문 Table 2의 3개 피처(DD hl=10, Sortino hl=20·60). `example`: 저장소 나스닥 예제의 9개 피처(hl 5·20·60 × 수익률·log DD·Sortino). `extra`: `example` 9개 + 수익률·변동성 파생 25개 + 원 스케일 `DD_10`·`DD_20`·`DD_60` = 37개 (3-1 참고) |
| `--log-dd` | 꺼짐 | 원 스케일 downside deviation(`paper`의 `DD_10`, `extra`의 `DD_10/20/60`)을 로그 변환. `extra`에서는 `DD_20`·`DD_60`이 `example`의 `DD-log_20/60`과 같아져 빠지므로 35개가 됩니다 |
...
`example`의 9개 피처에, 수익률과 변동성에서 흔히 쓰는 파생 변수 25개와 원 스케일 downside deviation 3개(`DD_10`·`DD_20`·`DD_60`)를 더한 **37개 피처** 세트입니다. 세 피처 세트를 모두 포함하므로 `--pin-feature paper`·`--pin-feature example`이 빠짐없이 적용됩니다(3-6 참고). 수익률 시계열 하나만으로 만들 수 있는 통계를 폭넓게 깔아 두고 모델이 고르게 하는 용도이므로, **`--model sjm`과 함께 쓰는 것을 권장합니다.**
...
| `DD_10`, `DD_20`, `DD_60` | 3 | 반감기 10·20·60일 downside deviation, **원(raw) 스케일**. `DD_10`은 논문 Table 2의 피처입니다. `--log-dd`를 주면 `DD-log_10` 하나만 남습니다(아래 참고) |
...
- **`DD_10`은 `paper` 세트에서 그대로 가져온 열입니다.** 반감기 10일은 `example`·`extra`의 어떤 피처도 쓰지 않는 horizon이라, 이 열이 없으면 `--feature-set extra`에서는 논문의 대표 피처를 고정할 수 없었습니다. `paper` 세트와 이름·정의를 정확히 맞추려고 기본값을 원(raw) 스케일로 두었습니다.
- ⚠️ **`DD_20`·`DD_60`은 `DD-log_20`·`DD-log_60`과 같은 통계입니다** — 로그를 씌우면 정확히 같은 값이라(`log(DD_20) == DD-log_20`), 정보량은 동일하고 스케일만 다릅니다. 군집 거리(윈저라이징·표준화 후 유클리드)는 스케일에 따라 달라지므로 완전한 중복은 아니지만, `sjm`에서는 사실상 같은 정보를 두 열이 나눠 갖고 예산(`--max-feats`)을 함께 소모합니다. 원 스케일 DD를 꼭 쓰려는 게 아니라면 `DD_10`만 고정하고 나머지는 로그 열에 맡기는 편이 낫습니다.
- **`--log-dd`를 주면 `DD_20`·`DD_60`은 만들지 않습니다.** 로그 스케일에서는 `example`이 이미 만든 `DD-log_20`·`DD-log_60`과 이름·값이 완전히 같아지기 때문입니다. 그래서 `--feature-set extra --log-dd`는 `DD-log_10` 하나만 더해 35개가 되고, 이때 `--pin-feature DD_20`은 열이 없다는 `KeyError`를 냅니다(대신 `DD-log_20`을 고정하세요).
- 다른 반감기의 원 스케일 DD가 필요하면 `features.EXTRA_DD_HLS`에 추가하면 됩니다.
...
```bash
# paper 3개 피처(DD_10·sortino_20·sortino_60)를 고정하고 나머지 34개는 sjm이 알아서 고르게
# (사실상 "논문 피처 + 보조 피처")
...
- **`--feature-set`에 없는 열은 고정할 수 없습니다.** 예를 들어 `--feature-set example --pin-feature paper`는 두 세트가 공유하는 `sortino_20`·`sortino_60`만 고정하고, paper의 `DD_10`(반감기 10일)은 example 세트가 만들지 않으므로 경고와 함께 건너뜁니다. 세 피처를 모두 고정하려면 `--feature-set paper`나 `--feature-set extra`를 쓰세요 — `extra`는 `example`의 9개와 `paper`의 `DD_10`을 모두 포함하므로 `--pin-feature paper`·`--pin-feature example`이 빠짐없이 적용되고, `--pin-feature DD_10`(또는 `DD_20`·`DD_60`)처럼 개별 열만 지정하는 것도 그대로 동작합니다. `--log-dd`를 같이 주면 이름이 `DD-log_10`으로 바뀌고 `DD_20`·`DD_60`은 아예 만들어지지 않으니(3-1 참고) 개별 지정도 그에 맞춰야 합니다.
