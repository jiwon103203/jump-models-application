...
1. 초과수익률 계산 →
2. 지수가중이동(EWM) downside deviation·Sortino ratio 피처 생성 →
3. **6개월마다(1월·7월 첫 영업일) 3000거래일 학습창으로 점프 모델 재추정** + 재추정 사이 구간은 온라인 추론 →
4. 0/1 전략 백테스트(거래비용·거래지연 반영)
...
- **λ 선택**: 논문 §3.4.3의 시계열 교차검증(매월, 8년 검증창, Sharpe 최대화)은 구현하지 않았습니다. `--jump-penalty`로 고정값을 주며, 논문의 대표값은 50.0입니다.
...
`make_sample_data.py`로 만든 샘플(나스닥 100 종가 + 합성 무위험금리, 1989–2024, λ=50, 3000일 학습창, 1일 지연, 10bp)의 출력입니다.

```
온라인 레짐 구간: 1989-01-03 ~ 2024-09-27 (9004거래일)
bear 레짐 비중: 20.3%, 레짐 전환 31회 (연 0.87회)
             B & H  JM 0/1
Return       14.1%   13.9%
Volatility   26.3%   19.7%
Sharpe        0.53    0.62
MDD         -82.9%  -46.7%
Calmar        0.17    0.26
ES_0.05      -3.9%   -3.0%
...
무위험금리가 합성 데이터라 논문의 S&P 500 결과와 직접 비교할 수는 없지만, 레짐 전환 빈도(연 0.87회)와 회전율·레버리지 수준은 논문 Table 3·4의 λ=50 구간과 같은 범위에 있습니다.
